...
class Buffalo {
  ...
  writeBuffer(values, length) {
    if (values.length !== length) {
      throw new Error("Length of values: '".concat(values, "' is not consitent with expected length '").concat(length, "'"));
    }
  ...
  write(type, value, options) {
    ...
    // }
      ...
      throw new Error("Write for '".concat(type, "' not available"));
    }
  }
  ...
  read(type, options) {
    if (type === 'UINT8') {
      ...
      throw new Error("Read for '".concat(type, "' not available"));
    }
  }
